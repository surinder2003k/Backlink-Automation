...
export function DateTimePicker({ value, onChange, min }: DateTimePickerProps) {
  ...
  const handleDateSelect = (d: number) => {
    ...
    const iso = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}T${h}:${m}`;
    onChange(iso);
  };
  ...
  const handleTimeChange = () => {
    ...
    const h = timeH.padStart(2, "0");
    const m = timeM.padStart(2, "0");
    const iso = `${selectedDate.getFullYear()}-${String(selectedDate.getMonth() + 1).padStart(2, "0")}-${String(selectedDate.getDate()).padStart(2, "0")}T${h}:${m}`;
    onChange(iso);
  };
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full mt-1 left-0 z-50 w-72 rounded-md border border-cyber-border bg-cyber-bg shadow-xl">
          {/* Month/Year nav */}
          ...
          <div className="flex items-center gap-2 px-3 py-2 border-t border-cyber-border">
            ...
            <select
              value={timeH}
              onChange={(e) => { setTimeH(e.target.value); setTimeout(handleTimeChange, 0); }}
            ...
            <select
              value={timeM}
              onChange={(e) => { setTimeM(e.target.value); setTimeout(handleTimeChange, 0); }}
              className="bg-cyber-card border border-cyber-border rounded text-sm text-cyber-text px-2 py-1 font-mono"
            >
